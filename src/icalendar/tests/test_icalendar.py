...
class IcalendarTestCase(unittest.TestCase):
    ...
    def test_contentline_class(self):
        ...
        assert Contentline("REFRESH - INTERVAL; VALUE = DURATION:PT48H").parts() == (
            "REFRESH-INTERVAL",
    ...
    def test_fold_line_keeps_escape_sequence(self):
        r"""_foldline must not split a TEXT escape sequence across a fold.
        ...
        (``\\``, ``\n``, ``\N``, ``\;``, ``\,``). This test ensures
        _foldline keeps the escape sequence intact on the same side of
        the fold, so downstream parsers always see a complete escape.
        """
        assert _foldline("foo\\nbar") == "foo\\nbar"
